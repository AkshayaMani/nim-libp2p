...
proc validateAndRelay(
  ...
  try:
    ...
    g.broadcast(
      toSendPeers, RPCMsg(messages: @[msg]), isHighPriority = false, useMix = false
    )
...
method publish*(
    g: GossipSub, topic: string, data: seq[byte], useMix: bool = false
): Future[int] {.async.} =
  logScope:
    topic
  ...
  if useMix:
    ...
    for i in 0 .. 4:
      ...
      let
        value = cast[uint64](bytes)
        peerIndex = int(value mod uint64(allPeers.len))
      peers.incl(allPeers[peerIndex])
  else:
  ...
  g.broadcast(peers, RPCMsg(messages: @[msg]), isHighPriority = true, useMix = useMix)
